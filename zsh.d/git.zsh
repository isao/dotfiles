...
__fzf-git-log() {
    ...
    git log --format='%C(auto)%h %s %C(blue)@%al' --color=always $@ \
        | fzf --accept-nth=1 --ansi --no-sort --preview='git show --color --stat --decorate --pretty=fuller -p {1}' \
        | xargs
}
...
fzf-git-modified-widget() {
    LBUFFER+="$(git status --short | fzf -n2  --accept-nth=2 --exit-0 --preview 'git diff --color {2}' | xargs)"
    zle redisplay
}
...
fzf-git-pick-commit() {
    LBUFFER+="$(__fzf-git-log --all --no-merges -9999)"
    zle redisplay
}
...
fzf-git-fixup-commit() {
    LBUFFER+="$(__fzf-git-log $(git log --merges --format=%h -1)..HEAD)"
    zle redisplay
}
