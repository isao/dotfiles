...
if (! whence eza >/dev/null) {
    alias ll='ls -lh'       # -h human readable sizes
    alias ll.='ll -d .?*'   # ll dot files & dirs only
    alias lld='ll -dl */'   # ll just directories
}
...
alias showfunctions='functions + | rg "^[a-z]"'
alias showkeys='rg --no-line-number -o "^bindkey .+$" ~/.zshrc $myzshd/*.zsh'
